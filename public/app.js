document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- API Helper (מתוקן) ---
    // (הפונקציות הללו מטפלות ב-401 אוטומטית ומוסיפות את credentials: 'include')
    ...
    // פונקציית POST (ללא FormData)
    async function postData(url, body, successMessage, callback) {
         const res = await fetch(url, {
            method: 'POST',
    ...
    // פונקציית PUT
    async function putData(url, body, successMessage, callback) {
         const res = await fetch(url, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            // 🛑 התיקון הקריטי
    ...
    async function logout() {
        // 🛑 שימוש ב-fetch ישיר כי אין צורך ב-credentials ב-logout (הוא מנקה את ה-Session)
        ...
        try {
            ...
            if (canPost) {
                try {
                    ...
                    // מתעלם משגיאת 401 קטנה, כי היא לא מונעת את טעינת הפוסטים הראשיים
    ...
    async function loadAssignments() {
        ...
        try {
            // 🛑 שימוש ב-get המתוקן
            state.assignments = await get('/api/assignments'); 
            ...
            const assignmentsHtml = state.assignments.length > 0
                ? state.assignments.map(a => {
                    ...
                    if (isStudent) {
                        submissionHtml = `
                            <form class="submit-assignment-form" data-id="${a._id}">
                                <div class="form-group">
                                    <label for="submission-${a._id}">📤 הגש מטלה (קובץ):</label>
                                    <input type="file" id="submission-${a._id}" name="submissionFile" required>
                                </div>
                                ...
                                <ul class="submissions-list">
                                    ${a.submissions.map(sub => `
                                        <li>
                                            ...
                                            <a href="/api/assignments/download/${a._id}/${sub.studentId}" target="_blank">📥 הורד קובץ</a>
                    ...
                    return `
                        <article class="item-card">
                            <div class="item-header">
                                ...
                                ${canManage ?
                                `<button class="btn-danger btn-small" data-action="delete-assignment" data-id="${a._id}">🗑️ מחק</button>` 
                                : ''}
                            </div>
        ...
        try {
            ...
            const usersHtml = state.users.map(user => {
                ...
                const actionsHtml = protectedUser
                    ...
                    : `
                        <button class="btn-primary btn-small" data-action="show-edit-user-form" data-id="${user._id}" data-user='${JSON.stringify(user)}'>✏️ ערוך</button>
                        <button class="btn-danger btn-small" data-action="delete-user" data-id="${user._id}">🗑️ מחק</button>
                    `;
    ...
    async function loadClasses() {
        ...
        try {
            ...
            const teacherOptions = teachers.map(t => `<option value="${t._id}">${t.fullname}</option>`).join('');

            // (חדש) נכין רשימת תלמידים לבחירה
            const students = state.users.filter(u => u.role === 'student');
            const studentOptions = students.map(s => `<option value="${s._id}">${s.fullname} (ID: ${s._id})</option>`).join('');

            const classesHtml = state.classes.map(c => {
                const teacher = state.users.find(u => u._id === c.teacherId);
                
                // (חדש) בדיקה אם המשתמש הנוכחי יכול לנהל את הכיתה (מנהל או המורה שלה)
                const canManageClass = state.currentUser.role === 'admin' || c.teacherId === state.currentUser._id;
                
                // (חדש) רשימת תלמידים בכיתה
                const studentsInClass = c.students.map(studentId => {
                    const s = state.users.find(u => u._id === studentId);
                    // (חדש) הוספת כפתור הסרה לתלמיד
                    const removeButton = canManageClass ? 
                        `<button class="btn-danger btn-small" data-action="remove-student-from-class" data-class-id="${c.id}" data-student-id="${s._id}">❌ הסר</button>` : '';

                    return s ? `<li>${s.fullname} ${removeButton}</li>` : `<li>תלמיד לא ידוע (ID: ${studentId})</li>`;
                }).join('');
    ...
    function loadProfile() {
        // (חדש) תמיכה בריבוי כיתות
        const { fullname, email, role, classIds, _id } = state.currentUser;
        render(`
            <section class="view">
                ...
                <ul class="details-list">
                    ...
                    <li><strong>🆔 מזהה משתמש:</strong> ${_id}</li>
                </ul>
            </section>
        `);
    }
    ...
    function handleContentClick(e) {
        ...
        if (action === 'show-edit-user-form') {
            ...
            document.getElementById('edit-user-id').value = user._id; // משתמש ב-_id
        ...
        if (action === 'delete-user') {
            if (confirm('האם אתה בטוח שברצונך למחוק משתמש זה?')) {
                deleteData(`/api/users/${id}`, 'משתמש נמחק בהצלחה', loadUsers);
            }
        }
        if (action === 'delete-post') {
            if (confirm('האם אתה בטוח שברצונך למחוק הודעה זו?')) {
                deleteData(`/api/posts/${id}`, 'הודעה נמחקה בהצלחה', loadPosts);
            }
        }
        if (action === 'delete-assignment') {
            if (confirm('האם אתה בטוח שברצונך למחוק משימה זו? פעולה זו תמחק גם את כל ההגשות של התלמידים.')) {
                deleteData(`/api/assignments/${id}`, 'משימה נמחקה בהצלחה', loadAssignments);
            }
        }
        // (חדש) מחיקת כיתה
        if (action === 'delete-class') {
             if (confirm('האם אתה בטוח שברצונך למחוק כיתה זו? פעולה זו תסיר את כל התלמידים המשויכים אליה.')) {
                deleteData(`/api/classes/${id}`, 'כיתה נמחקה בהצלחה', loadClasses);
            }
        }
        
        // (חדש) הסרת תלמיד מכיתה
        if (action === 'remove-student-from-class') {
             const classId = e.target.dataset.classId;
             const studentId = e.target.dataset.studentId;
             if (confirm('האם אתה בטוח שברצונך להסיר תלמיד זה מהכיתה?')) {
                // 🛑 שימוש ב-deleteData (שולח credentials: 'include')
                deleteData(`/api/classes/${classId}/students/${studentId}`, 'תלמיד הוסר בהצלחה', loadClasses);
            }
        }
    }
    ...
    async function handleFormSubmit(e) {
        ...
        try {
            if (form.id === 'add-user-form') {
                // (חדש) קבלת מערך כיתות
                const selectedClasses = Array.from(form['user-classId'].selectedOptions).map(opt => parseInt(opt.value));
                ...
                // 🛑 שימוש ב-postData המתוקן
                await postData('/api/users', body, 'משתמש נוצר בהצלחה', loadUsers);
            }
            
            // (חדש) טופס עריכת משתמש
            if (form.id === 'edit-user-form') {
                const id = form['edit-user-id'].value;
                const selectedClasses = Array.from(form['edit-user-classId'].selectedOptions).map(opt => parseInt(opt.value));
                if (selectedClasses.length > 10) {
                    return showNotification('לא ניתן לבחור יותר מ-10 כיתות.', 'error');
                }
                const body = {
                ...
                // 🛑 שימוש ב-putData המתוקן
                await putData(`/api/users/${id}`, body, 'משתמש עודכן בהצלחה', loadUsers);
            }
            ...
            if (form.id === 'add-class-form') {
                ...
                if (!teacherId && state.currentUser.role === 'teacher') {
                    teacherId = state.currentUser._id;
                }
                ...
                // 🛑 שימוש ב-postData המתוקן
                await postData('/api/classes', body, 'כיתה נוצרה בהצלחה', loadClasses);
            }
            ...
            if (form.id === 'add-post-form') {
                ...
                const body = {
                    ...
                    classId: isPrivate ? parseInt(classId) : null
                };
                // 🛑 שימוש ב-postData המתוקן
                await postData('/api/posts', body, 'הודעה פורסמה בהצלחה', loadPosts);
            }
            ...
            if (form.id === 'add-assignment-form') {
                ...
                // 🛑 שימוש ב-postData המתוקן
                await postData('/api/assignments', body, 'משימה נוצרה בהצלחה', loadAssignments);
            }
            ...
            if (form.classList.contains('add-student-to-class-form')) {
                ...
                // 🛑 שימוש ב-postData המתוקן
                await postData(`/api/classes/${classId}/students`, body, 'תלמיד נוסף לכיתה בהצלחה', loadClasses);
            }
            ...
            if (form.classList.contains('submit-assignment-form')) {
                ...
                const res = await fetch(`/api/assignments/${id}/submit`, {
                    method: 'POST',
                    // 🛑 התיקון הקריטי: קריאות עם FormData לא משתמשות ב-headers: 'application/json'
                    credentials: 'include', 
                    body: formData 
                });
    ...
    // --- Start ---
    initializeApp();
}); // 🛑 סוגר את הפונקציה האנונימית של EventListener
